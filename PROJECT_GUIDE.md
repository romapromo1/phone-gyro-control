...
* **Как играть**:
  ...
  3. Игрок наклоняет смартфон вперед/назад и влево/вправо, а также может поворачивать его горизонтально вокруг вертикальной оси (Yaw).
  4. Повороты и наклоны смартфона транслируются на визуальный наклон и вращение лабиринта на экране (лабиринт крутится вокруг своей вертикальной оси Y вслед за поворотом телефона). Шар катится под действием наклона гравитации.
...
* При наклоне и повороте телефона группа `mazeContainer` вращается по трем осям: Pitch (тангаж), Roll (крен) и Yaw (рыскание/поворот вокруг вертикальной оси Y). Это создает визуальный эффект полного 3D-вращения и наклона короба с лабиринтом перед глазами пользователя.
...
### 3.2. Вращение по оси Y/Yaw (Синхронизация физики и графики при повороте телефона)
* Когда пользователь поворачивает телефон в руках горизонтально, лабиринт на экране вращается вокруг вертикальной оси Y.
* Чтобы физика оставалась точной (например, если лабиринт повернут на 90 градусов, наклон вперед должен катить шар «налево» с точки зрения неподвижной физической сетки), вектор гравитации в физическом мире динамически вращается по формуле поворота на угол рыскания (`currentYaw`):
```typescript
...
```
Это полностью синхронизирует визуальное вращение сцены и реальное физическое поведение катящегося шара.
...
Система поддерживает два режима работы в зависимости от окружения:

### 4.1. Локальный режим (Local Development / On-Premise)
1. **Автоматический SSL**: Сервер `server.js` генерирует самоподписанный 2048-битный SSL-сертификат для локального IP-адреса ПК. Мобильные браузеры дают доступ к API гироскопа (`DeviceOrientation`) **только** по безопасному протоколу `https://`.
2. **Паринг по QR**: На экране Holobox отрисовывается QR-код, содержащий локальный адрес `https://<локальный-IP-ПК>:3000/controller.html`.
3. **Сеть**: Смартфон и ПК **обязательно** должны находиться в одной Wi-Fi сети для прямой маршрутизации запросов.

### 4.2. Облачный режим (Деплой на Render.com)
1. **SSL-терминация**: Render.com автоматически предоставляет защищенный HTTPS-сертификат (Let's Encrypt) на внешних адресах. Локально `server.js` запускается в обычном HTTP-режиме (определяя флаг `NODE_ENV === 'production'`), а защищенный протокол обслуживается внешним роутером Render.
2. **Динамический роутинг**: Метод `/api/server-info` автоматически считывает HTTP-заголовки `x-forwarded-host` и `x-forwarded-proto` для генерации корректного публичного QR-кода вида `https://<имя-вашего-проекта>.onrender.com/controller.html`.
3. **Облачный мост (Cloud Relay)**: Благодаря публичному веб-сокету на Render.com, смартфону **не требуется** находиться в одной Wi-Fi сети с ПК. Управление работает из любой точки мира через мобильный интернет, а сервер выступает быстрым сетевым мостом.

### 4.3. Оптимизация протокола
* **Безопасность подключения**: При подключении нового телефона старая сессия автоматически отключается на сервере для избежания конфликта одновременного управления.
* **Передача данных**: Данные отправляются в режиме `volatile` (без подтверждения доставки), обеспечивая задержку менее 8 миллисекунд.
